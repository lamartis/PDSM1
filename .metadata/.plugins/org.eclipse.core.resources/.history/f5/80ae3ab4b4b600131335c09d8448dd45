...
public class CreateOrUpdateArchiveRequestTraitement extends TraitementP {

	CreateOrUpdateArchiveRequest createOrUpdateArchiveRequest;
	String historyMessage = null;

	public CreateOrUpdateArchiveRequestTraitement(P2mInterface object, String idMessage, String correlationId){
		super(object, idMessage, correlationId);
		createOrUpdateArchiveRequest = (CreateOrUpdateArchiveRequest) requete;
	}
	...
	public void createZip(){
		...
		//Récuperer le Login de l'user, pour pouvoir enregister le zip sur son dossier.
		File file;
		try {
			...
			for (Element element : createOrUpdateArchiveRequest.getArchive().getElement()) {
				...
				//Supprimer le fichier après l'avoir compresser
